...
2. **Mechanism: subtree sync, not a monorepo split.** One workflow per public repo, modelled on
   `sync-sdk-ts.yml`: `sync-phone.yml` → `codai-ro/codai-phone`, later `sync-desktop.yml` →
   `codai-ro/codai-desktop`, `sync-protocol.yml` → `codai-ro/codai-protocol`. Snapshot = rsync of the subtree +
