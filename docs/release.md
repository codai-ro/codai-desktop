...
- The updater endpoint `https://codai.ro/api/desktop/latest.json` is a
   ...
   ```json
   {
     ...
     "platforms": {
       "windows-x86_64": {
         ...
         "url": "https://codai.ro/api/desktop/download/codai_0.1.1_x64-setup.exe"
       }
     }
   }
   ```
...
- The three version files must agree, and must match what is actually
  published.
- After publishing, `curl https://codai.ro/api/desktop/latest.json` must
